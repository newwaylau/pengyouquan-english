-- ======================================================
-- 错题表扩展：增加"已掌握"标记、冗余存储翻译和剧集名
-- ======================================================

ALTER TABLE wrong_sentences
    ADD COLUMN is_mastered TINYINT(1) NOT NULL DEFAULT 0 COMMENT '是否已掌握（标记而非删除）',
    ADD COLUMN chinese_translation VARCHAR(500) DEFAULT '' COMMENT '中文翻译（冗余存储，方便显示）',
    ADD COLUMN show_name VARCHAR(255) DEFAULT '' COMMENT '剧集名（冗余存储，方便筛选）',
...
SET ws.show_name = COALESCE(sh.name, ''),
    ws.chinese_translation = COALESCE(s.text, '');
